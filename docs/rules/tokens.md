...
**`DESIGN.md` is the brand contract: what was decided, and why. It must not contain a single colour value, pixel measurement or duration.** Those live in `global.css`, which is the only place they can be true — a value written in two places drifts in one of them, and it is always the copy that nobody renders from.
...
|---|---|
| Why this palette, this shape language, this motion character | The hex codes and durations → `global.css` |
| Hard constraints — "no radius", "one intent action per screen" | What a component looks like → `/components` and its `.mdx` |
| What is deliberately *not* done, and why | Prop tables and measurements → the component itself |
| Which decisions are still open for this project | — |
