...
**`docs:check`** is the documentation half of the gate ([guardrails.docs-check]): it fails the build on a citation to a rule id that doesn't exist, on a stale `STANDARDS.md`, on a broken relative link, and on any surviving `§` section reference.
...
- names `DESIGN.md` (tokens/brand) and `STANDARDS.md` as **authoritative**, and defers to them rather than restating rules ([principles]);
